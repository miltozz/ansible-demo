            ...
            steps {
                script {
                    ...
                    withCredentials([sshUserPrivateKey(credentialsId: 'new-ans-server-key', keyFileVariable: 'keyfile', usernameVariable: 'username')]) {
                        ...
                        sshCommand remote: remote, command: 'export A1=alpha1; export TEST_ENV=${env.TEST1}; export PATH=$PATH:/home/ubuntu/.local/bin; export AWS_ACCESS_KEY_ID=${AWS_ACCESS_KEY_ID}; export AWS_SECRET_ACCESS_KEY=${AWS_SECRET_ACCESS_KEY}; ansible-inventory -i dynamic_inv_aws_ec2.yml --graph; echo $A1; echo $AWS_ACCESS_KEY_ID; echo $TEST_ENV; printenv'
